...
int main() {
    ...
    if(d1 < 2*r && d2 < 2*r)
    {
        ...
        {
            flag1 = true;
            y01 = sqrt(4*r*r-d1*d1);
            x01 = d1;
            double tanab,tanb;
            tanab = yy*1.0/xx;
            tanb = x01*1.0/y01;
            tana = (tanab-tanb)/(1+tanab*tanb);
            d2 = (tana*emt[2].x-emt[2].y-tana*emt[1].x+emt[1].y)/sqrt(tana*tana + 1);
            if(d2 < 2*r) flag4 = true;
        }else if(dis1>dis2){
            flag2 = true;
            y01 = sqrt(4*r*r-d2*d2);
            x01 = d2;
            double tanab,tanb;
            tanab = yy*1.0/xx;
            tanb = x01*1.0/y01;
            tana = (tanab-tanb)/(1+tanab*tanb);
            d1 = (tana*emt[1].x-emt[1].y-tana*emt[2].x+emt[2].y)/sqrt(tana*tana + 1);
